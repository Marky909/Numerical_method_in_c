...
int main() {
    ...
    // Calculate s for backward interpolation
    s = (xp - x[n - 1]) / h;

    // Initialize the backward difference array
    for (i = 0; i < n; i++) {
        bd[i] = fx[i];
    }
    ...
    // Calculate the interpolated value using backward formula
    v = bd[n - 1]; // Initialize with the last term
    for (i = 1; i < n; i++) {
        p = 1; // Initialize product term
        for (k = 1; k <= i; k++) {
            p = p * (s + k - 1); // Adjust for backward difference
        }
        v = v + (bd[n - 1 - i] * p) / fact(i); // Add term to interpolated value
    }
